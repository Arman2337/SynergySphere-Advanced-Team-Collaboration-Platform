    ...
    <section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {myTasks.length > 0 ? myTasks.map(task => (
          <Link to={`/project/${task.project._id}`} key={task._id} className="block hover:-translate-y-1 transition-transform">
            <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden shadow-lg h-full flex flex-col">
              <div className="w-full h-32 bg-gray-800 flex items-center justify-center text-emerald-500 text-2xl font-bold">
                {task.title.substring(0, 2).toUpperCase()}
              </div>
              <div className="p-4 flex-grow flex flex-col justify-between">
                {/* ✅ Task Title */}
                <h3 className="text-lg font-semibold text-white mb-1 truncate">{task.title}</h3>
                
                {/* ✅ Project Name */}
                <p className="text-sm text-emerald-400 mb-2">Project: {task.project.name}</p>
                
                {/* ✅ Status Badge */}
                <span className={`px-2 py-1 text-xs font-bold w-max rounded-full ${
          ...
          <p className="text-gray-500 col-span-full">You have no tasks assigned to you.</p>
        )}
      </div>
    </section>
  );
