...
import { getProjects, createProject } from '../api/apiService';
import Modal from '../components/Modal'; // 👈 Import the Modal component

// A simple SVG icon component for reusability (from your original code)
                        ...
                        {projects.map(project => (
                            <Link to={`/project/${project._id}`} key={project._id} className="block hover:-translate-y-1 transition-transform">
                                <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden shadow-lg h-full flex flex-col">
                                    <div className="w-full h-48 bg-gray-800 flex items-center justify-center text-emerald-500 text-3xl font-bold">
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Create a New Project">
                <form onSubmit={handleCreateProject} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-4 pt-4">
                        <button type="button" onClick={() => setIsModalOpen(false)} className="py-2 px-4 text-gray-300 rounded-lg hover:bg-gray-700 transition-colors">
                            Cancel
                        </button>
                        <button type="submit" className="bg-emerald-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-emerald-700 transition-colors">
                            Create Project
                        </button>
                    </div>
                </form>
