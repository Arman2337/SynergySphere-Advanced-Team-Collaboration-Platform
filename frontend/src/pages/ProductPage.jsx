...
const TaskColumn = ({ title, tasks, color, onStatusChange, projectId }) => (
    <div className="bg-gray-950/50 rounded-xl p-4 w-full h-full">
        ...
        <div className="h-full overflow-y-auto">
            {tasks.map(task => <TaskCard key={task._id} task={task} onStatusChange={onStatusChange} projectId={projectId} />)}
        </div>
    </div>
);
        ...
        <div className={`flex h-screen font-sans bg-gray-950 text-gray-300`}>
            ...
            <main className="flex-1 bg-gray-950 p-8 overflow-y-auto flex flex-col">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-grow">
                    <TaskColumn title="To-Do" tasks={todoTasks} color="border-blue-500" onStatusChange={handleStatusChange} projectId={projectId} />
                    <TaskColumn title="In Progress" tasks={inProgressTasks} color="border-yellow-500" onStatusChange={handleStatusChange} projectId={projectId} />
                    <TaskColumn title="Done" tasks={doneTasks} color="border-green-500" onStatusChange={handleStatusChange} projectId={projectId} />
                 </div>
            </main>
        </div>
    );
